const fs=require('fs-jetpack');
const { conn } = require('../app/config');


exports.uploadFiles = (req,res) =>{
    var title = req.body.title
    if(Array.isArray(req.files.documents)){
        req?.files?.documents?.map((file,index)=>{
            var { originalname, destination, filename, size, mimetype } = file
            var thisfile=destination+'/'+filename 
            var name =originalname.split('.')[0]
            var ext = mimetype.split('/')[1]
            fs.move(thisfile, process.cwd() + `/server/uploads/files/${req.params.request_id}/${req.params.user_type}-${title}-${new Date().toDateString()}.` + ext, {overwrite: true})
            fs.remove(thisfile) 
        })
        res.send('Files Saved')
    }else{
        res.status(400).send('No Files Uploaded')
    }    
}


exports.deleteFile = (req,res) =>{
    fs.remove(process.cwd()+`/server/uploads/files/${req.params.id}/${req.params.filename}`)        
    res.send('File Deleted')
}

exports.getSingleFile = (req,res) =>{
    res.download(process.cwd()+`/server/uploads/files/${req.params.id}/${req.params.filename}`, req.params.filename)
}

exports.updateAvatar = async(req,res) =>{
    try{ 
        var a = fs.createWriteStream(process.cwd()+`/server/uploads/avatars/${req.user.id}.jpg`, {encoding: 'base64'}).write(req.body.image.split('base64,')[1])
        if(a) {res.send('Image Updated')}
        else{res.status(400).send('Avatar Not Saved')} 
    } 
    catch(err){ res.status(400).send(err.code) }
}

exports.getAvatar = async(req,res) =>{
    var file = `data:image/jpeg;base64,`;
    fs.readAsync(process.cwd()+`/server/uploads/avatars/${req.params.id}.jpg`, 'buffer').then(data => {
        res.send("data:image/jpeg;base64,"+Buffer.from(data).toString('base64'))
    }).catch(()=>res.send(file))
}

exports.getRequestAvatar = async(req,res) =>{
    var file = `data:image/jpeg;base64,`;
    conn('requests').where({id:req.params.id}).first().select('user_id').then(data=>{
        fs.readAsync(process.cwd()+`/server/uploads/avatars/${data.user_id}.jpg`, 'buffer').then(data => {
            res.send("data:image/jpeg;base64,"+Buffer.from(data).toString('base64'))
        })
        .catch(()=>res.send(file))
    }).catch(err=>{
        res.status(400).send(err.code)
    })
}

