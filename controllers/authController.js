...
exports.userSignin = async(req,res) => {
    ...
    if(remember_me === true) maxAge = 14


    conn('users').select().where({email:email, password:password, active:'', reset:''}).first()
        .then((data)=>{
            var token = jwt.sign({id:data.id, type:data.user_type}, process.env.SECRET_KEY, {expiresIn: `${maxAge}d`})
            ...
            res.cookie('auth', token, {secure: true, maxAge: 86400000*maxAge})
            jp.readAsync(process.cwd()+`/server/uploads/avatars/${data.id}.jpg`, 'buffer').then(data => {
                file = "data:image/jpeg;base64,"+Buffer.from(data).toString('base64')
                res.send({user: {name: `${data.firstname} ${data.lastname}`, user_type: `${data.user_type}`}, avatar: file})
            }).catch(err=> res.send({user: {name: `${data.firstname} ${data.lastname}`, user_type: `${data.user_type}`}, avatar: file}))
        })
        .catch(()=> res.status(401).send("Failed To Signin"))
}
...
exports.checkAuth = (...permissions) =>{
    return function (req,res,next) {
        ...
        if(req.cookies.CookieConsent === 'true'){
            try { 
                req.user = jwt.verify(req.cookies.auth, process.env.SECRET_KEY) 
                if(!permissions.includes(req.user.type)){
            ...
            res.status(400).send('You have not accepted Cookie Consent. You cannot access this resource!')
        }
    }
}