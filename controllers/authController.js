...
exports.userSignin = async(req,res) => {
    ...
    conn('users').select().where({email:email, password:password, active:'', reset:''}).first()
        .then((data)=>{
            var token = jwt.sign({id:data.id, type:data.user_type}, process.env.CRYPTO_KEY, {expiresIn: `${maxAge}d`})
...
exports.checkAuth = (...permissions) =>{
    return function (req,res,next) {
        ...
        if(req.cookies.CookieConsent === 'true'){
            try { 
                req.user = jwt.verify(req.cookies.auth, process.env.CRYPTO_KEY) 
